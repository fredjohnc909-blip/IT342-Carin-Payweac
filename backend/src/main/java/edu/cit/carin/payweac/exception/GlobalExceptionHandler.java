package edu.cit.carin.payweac.exception;

import edu.cit.carin.payweac.dto.ApiResponse;
import edu.cit.carin.payweac.service.AuthService;
...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(RentPaymentService.DuplicateRentPaymentException.class)
    public ResponseEntity<ApiResponse<Void>> handleDuplicateRent(RentPaymentService.DuplicateRentPaymentException ex) {
        ApiResponse<Void> response = ApiResponse.error("RENT-001", "Duplicate payment", ex.getMessage());
        return ResponseEntity.status(HttpStatus.CONFLICT).body(response);
    }
}
